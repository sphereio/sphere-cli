...
module Sphere
  describe 'Account' do
    ...
    describe '#login' do
      ...
      it 'just works' do
        Excon.stub(
          { :method => :post, :path => '/login', :body => 'email=someone%40example.com&password=secret' },
    ...
    describe '#signup' do
      it 'account already exists' do
        Excon.stub(
          { :method => :post, :path => '/signup', :body => 'name=me+and+i&email=someone%2B123%40example.com&password=secret' },
